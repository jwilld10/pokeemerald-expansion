...
# Safe clean that never deletes graphics/sound source assets (palettes, bpp, etc)
clean-safe: tidy clean-tools clean-check-tools clean-generated
	@echo "clean-safe complete (assets preserved)"
...
clean-assets:
	...
	@if [ "$(CLEAN_ASSETS)" = "1" ]; then \
	  ...
	  echo "Skipping asset deletion. (Run: make clean CLEAN_ASSETS=1 if you REALLY want to remove *.gbapal/*.4bpp/etc)"; \
	fi
