...
PY="$ROOT/tools/bw3g/png_to_gba_sprite_spaceworld_style.py"
FAMS="$ROOT/src/data/pokemon/species_info/bw3g_families.h"
OUTGFX="$ROOT/graphics/bw3g/pokemon"

if [[ ! -f "$FAMS" ]]; then
  echo "ERROR: missing $FAMS"
  exit 1
fi
...
# Parse sizes from bw3g_families.h:
...
#
# We’ll build a map: mon_dir_name -> frontWxH backWxH
declare -A FRONT
declare -A BACK

# Extract current species block name + sizes
# mon_dir is lowercased species base (e.g. "snivy") from the BW3G gfx folder names.
# In your BW3G folder you already have: graphics/bw3g/pokemon/<monname>/front.png
# So we key by that <monname> directory.
...
# find each species entry header with the name inside the []:
entries = re.split(r"\n----\s+\[([A-Z0-9_]+)\]\s+=\s+----\n", txt)
# entries: [pre, NAME1, BODY1, NAME2, BODY2, ...]
out = []
for i in range(1, len(entries), 2):
    name = entries[i]
    body = entries[i+1]
    m1 = re.search(r"\.frontPicSize\s*=\s*MON_COORDS_SIZE\((\d+)\s*,\s*(\d+)\)\s*,", body)
    m2 = re.search(r"\.backPicSize\s*=\s*MON_COORDS_SIZE\((\d+)\s*,\s*(\d+)\)\s*,", body)
    if not m1 or not m2:
        continue
    fw, fh = m1.group(1), m1.group(2)
    bw, bh = m2.group(1), m2.group(2)
    # SPECIES_FOO_BW3G -> "foo"
    base = name
    base = re.sub(r"^SPECIES_", "", base)
    base = re.sub(r"_BW3G$", "", base)
    base = base.lower()
    out.append(f"{base} {fw}x{fh} {bw}x{bh}")
print("\n".join(out))
PY

while read -r mon fsz bsz; do
  [[ -z "${mon:-}" ]] && continue
  FRONT["$mon"]="$fsz"
  BACK["$mon"]="$bsz"
done < /tmp/bw3g_sizes.map

echo "== Converting BW3G PNGs -> tiled 4bpp.lz (Spaceworld-style rules) =="
converted=0
skipped=0
...
for d in "$OUTGFX"/*; do
  ...
  if [[ -z "${FRONT[$mon]:-}" || -z "${BACK[$mon]:-}" ]]; then
    echo "SKIP: $mon (no size info in bw3g_families.h)"
    skipped=$((skipped+1))
    continue
  fi

  # Output goes into the same folder:
  python3 "$PY" "$front_png" "$back_png" "$d" "${FRONT[$mon]}" "${BACK[$mon]}" "$mon" || {
    echo "SKIP: $mon (converter error)"
    skipped=$((skipped+1))
    continue
  }
