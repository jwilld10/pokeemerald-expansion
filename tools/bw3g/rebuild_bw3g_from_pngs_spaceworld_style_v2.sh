...
REPO_ROOT="$(pwd)"
PNG_ROOT="${BW3G_PNG_ROOT:-$HOME/decomps/gb/BW3G/gfx/pokemon}"
FAMS="src/data/pokemon/species_info/bw3g_families.h"

OUT_ROOT="$REPO_ROOT/graphics/bw3g/pokemon"
PY="$REPO_ROOT/tools/bw3g/png_to_gba_sprite_spaceworld_v2.py"
...
if [[ ! -d "$PNG_ROOT" ]]; then
  echo "ERROR: PNG_ROOT not found: $PNG_ROOT"
  exit 1
fi
...
echo "== Converting BW3G PNGs -> tiled 4bpp.lz (spaceworld-style, alpha-safe) =="
python3 "$PY" "$PNG_ROOT" "$FAMS" "$OUT_ROOT"

echo
echo "== NOTE =="
echo "We wrote to: $OUT_ROOT/<mon>/{front.4bpp.lz, back.4bpp.lz, normal.gbapal}"
echo "This converter always produces 64x64 raw tiled data, but palette index0 is reserved for transparency only."
echo "SpeciesInfo can keep its original MON_COORDS_SIZE — the renderer should ignore the padded area."
